...
contract ControllerHubPolicyTest is Test {
    ...
    function setUp() public {
        ...
        asset = new TestERC20("ASSET","AST"); lst = new TestERC20("LST","LST");
        seize = new SeizeMock();
        ...
        ControllerHub.MarketParams memory p = ControllerHub.MarketParams({
            ...
            borrowCap: 1000 ether, kinkBps: 8000, slope1Ray: 1e16, slope2Ray: 2e16,
            baseRateRay: 0, lst: address(lst), vault: address(seize)
        });
        vm.prank(gov); hub.listMarket(address(asset), abi.encode(p));
        lst.mint(user, 1000 ether); vm.prank(user); hub.enterMarket(address(lst));
    }

    function testSetBorrowCapAndEnforcement() public {
        vm.prank(gov); hub.setBorrowCap(address(asset), 300 ether);
        vm.prank(user);
    ...
    function testPauseFlags() public {
        vm.prank(gov); hub.setPause(true, false);
        vm.expectRevert(abi.encodeWithSignature("InsufficientCollateral()"));
        vm.prank(user); hub.borrow(address(asset), 1 ether, 0);
        vm.prank(gov); hub.setPause(false, true);
    ...
    vm.prank(user); hub.borrow(address(asset), 500 ether, 0);
        ...
        vm.prank(gov); hub.setPause(false, false);
