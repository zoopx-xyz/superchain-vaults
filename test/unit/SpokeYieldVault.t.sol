...
contract MockAdapterRegistryVar {
    bool public allowed; uint256 public cap;
    constructor(bool a, uint256 c){ allowed=a; cap=c; }
    function set(bool a, uint256 c) external { allowed=a; cap=c; }
    function isAllowed(address) external view returns (bool) { return allowed; }
    function capOf(address) external view returns (uint256) { return cap; }
}
...
contract MockAdapterTarget {
    ...
    function deposit(uint256 assets, bytes calldata data) external returns (uint256) { emit DepositCalled(assets, data); return assets; }
    function withdraw(uint256 assets, bytes calldata data) external returns (uint256) { emit WithdrawCalled(assets, data); return assets; }
    function harvest(bytes calldata data) external returns (uint256) { emit HarvestCalled(data); return 0; }
}
...
contract TestERC20 is Test {
    ...
    function totalSupply() external view returns(uint256){return 0;} // for rewards tests compatibility
}

contract SpokeYieldVaultTest is Test {
    SpokeYieldVault vault;
    TestERC20 asset; SuperchainERC20 lst;
    ...
    function testAllocateDeallocateHarvestAndCaps() public {
        ...
        vm.prank(gov);
        vault2.initialize(IERC20(address(asset)), "Vault2", "vAST2", hub, gov, rebal, address(reg2), gov, 0, address(lst));
        ...
        vm.prank(gov);
        vault3.initialize(IERC20(address(asset)), "Vault3", "vAST3", hub, gov, rebal, address(reg3), gov, 0, address(lst));
