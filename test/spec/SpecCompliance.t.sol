...
contract SequencerMock {
    ...
    constructor(int256 u, uint256 t) {
        up = u;
        ts = t;
    }
...
contract SpecCompliance is Test {
    ...
    // topics
    bytes32 constant T_BORROW = keccak256("Borrow(address,address,uint256,uint256,uint256,uint256,bytes32)");
    bytes32 constant T_REPAY = keccak256("Repay(address,address,uint256,uint256,uint256,bytes32)");
    bytes32 constant T_LIQ = keccak256("Liquidate(address,address,address,uint256,address,uint256,uint256,bytes32)");
    bytes32 constant T_ACCRUED = keccak256("Accrued(address,uint256,uint256,uint256,uint256,uint256)");
    bytes32 constant T_ALLOC = keccak256("AdapterAllocated(address,address,uint256)");
    bytes32 constant T_HARV = keccak256("Harvest(address,address,uint256)");
    ...
    function setUp() public {
        ...
        superHub = new SuperVaultHub();
        superHub.initialize(address(0xdead), address(0xbeef), gov, relayer);
        ...
        // Keep initial LST price at parity; we'll lower it before liquidation
        aave = new AaveV3Adapter();
        velo = new VelodromeLPAdapter();
        aave.initialize(address(vault), address(asset), gov);
        velo.initialize(address(vault), address(asset), gov);
        vm.prank(gov);
        reg.setAdapter(address(aave), true, type(uint256).max);
        vm.prank(gov);
        reg.setAdapter(address(velo), true, type(uint256).max);
    ...
    function test_FunctionsExistBySelector() public pure {
        ...
        // consume to avoid "statement has no effect"
        bytes32 digest1 = keccak256(abi.encode(sels));
        require(digest1 != bytes32(0) || true, "sink sels");

        // Spoke hooks
        bytes4 onRemoteCreditSel = SpokeYieldVault.onRemoteCredit.selector;
        bytes4 requestRemoteLiquiditySel = SpokeYieldVault.requestRemoteLiquidity.selector;
        bytes4 payOutBorrowSel = SpokeYieldVault.payOutBorrow.selector;
        bytes4 onSeizeSharesSel = SpokeYieldVault.onSeizeShares.selector;
        bytes4[4] memory spokeSels = [onRemoteCreditSel, requestRemoteLiquiditySel, payOutBorrowSel, onSeizeSharesSel];
        bytes32 digest2 = keccak256(abi.encode(spokeSels));
        require(digest2 != bytes32(0) || true, "sink spokeSels");

        // Adapter allowlist + send/auth
        bytes4[4] memory adapterSels = [
        ...
        bytes32 digest3 = keccak256(abi.encode(adapterSels));
        require(digest3 != bytes32(0) || true, "sink adapterSels");
    }
    ...
    function test_EventsAndGuards_HappyPath() public {
        ...
        emit AdapterAllocated(address(aave), address(asset), 10 ether);
        vm.prank(rebal);
        vault.allocateToAdapter(address(aave), 10 ether, "");

        // Harvest adapter -> AdapterHarvest
        vm.expectEmit(true, true, false, true);
        emit Harvest(address(aave), address(asset), 0);
        vm.prank(rebal);
