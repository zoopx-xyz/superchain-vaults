...
contract MockShareToken {
    mapping(address=>uint256) public balanceOf; uint256 public ts;
    function totalSupply() external view returns (uint256) { return ts; }
    function mint(address to, uint256 a) external { balanceOf[to] += a; ts += a; }
}

contract EmissionsAndRewardsTest is Test {
    EmissionsController ec; PerChainRewardsDistributor rd;
    MockERC20 zpx; MockShareToken lst;
    address gov = address(0xA11CE);
    address user = address(0xBEEF);

    function setUp() public {
        ec = new EmissionsController(); ec.initialize(gov);
        ...
        vm.prank(gov); ec.setEpochCap(1_000 ether);
        vm.prank(gov); ec.setPerChainCap(block.chainid, 700 ether);
