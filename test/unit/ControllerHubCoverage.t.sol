...
contract ControllerHubCoverageTest is Test {
    ...
    function setUp() public {
        ...
        asset = new ERC20MockCheap("ASSET", "AST");
        lst = new ERC20MockCheap("LST", "LST");
    ...
    function testInvalidParamsOnListAndSet() public {
        ControllerHub.MarketParams memory bad1 = ControllerHub.MarketParams({
            ltvBps: 7000,
            liqThresholdBps: 6000,
            reserveFactorBps: 0,
            borrowCap: 0,
        ...
        vm.prank(gov);
        hub.listMarket(address(0x1), abi.encode(bad1));
        ControllerHub.MarketParams memory bad2 = ControllerHub.MarketParams({
            ltvBps: 1000,
            liqThresholdBps: 2000,
            reserveFactorBps: 0,
            borrowCap: 0,
            kinkBps: 0,
            slope1Ray: 0,
    ...
    function testRepayReducesDebtAndBorrows() public {
        vm.prank(user);
        hub.borrow(address(asset), 1_000 ether, 0);
        uint256 debtBefore = hub.currentDebt(user, address(asset));
        (ControllerHub.MarketState memory sBefore,,) = hub.marketStateExtended(address(asset));
        vm.prank(user);
        hub.repay(address(asset), 100 ether, 0);
    ...
    function testHealthFactorDefaultMax() public view {
        ...
        // With no debt, healthFactor() returns 1e18 by design for finite comparisons
        assertEq(hf, 1e18);
    }
}
